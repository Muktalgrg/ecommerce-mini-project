...
export class CheckoutComponent implements OnInit {
  ...
  /**
   * Validation:
   * dirty: when the user changes the field 
   * touched: when the field loses focus
   */
  ...
  ngOnInit(): void {
    ...
    this.luv2ShopFormService.getCreditCardYears().subscribe(
      data => {
        console.log("Retrievd credit card years: "+JSON.stringify(data));
  ...
  */
  resetCart(){
    // reset the cart data and the form and navigate back to product page.
  ...
  handleMonthsAndYears(){
    ...
    const selectedYear: number = new Number(creditCardFormGroup.value.expirationYear).valueOf();
